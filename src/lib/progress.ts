...
import { TRACK_CATEGORY_KEYS } from "@/lib/track-categories";
...
export const PROGRESS_MODE_OPTIONS: { value: TenantProgressMode; title: string; hint: string }[] = [
  { value: "teacher", title: "المعلمة", hint: "لكل معلمة حلقاتها تُدخل الأنصبة والتقدم والحضور لطالباتها." },
  { value: "supervisor", title: "المشرفة", hint: "المشرفة فقط تدخل الأنصبة والتقدم والحضور على مستوى المقرأة." },
  { value: "both", title: "المعلمة والمشرفة", hint: "كلتاهما قادرتان على الإدخال." },
];

/** تصدير ملف إكسل وتنزيله من المتصفح */
async function downloadWorkbook(filename: string, build: (ExcelJS: typeof import("exceljs").default) => void) {
...
type TrackExport = { name: string; category: string; oruj: number; absences: number };
...
export async function exportReportExcel(opts: {
  ...
  await downloadWorkbook(`تقرير-${opts.madrasa}.xlsx`, (ExcelJS) => {
    const ws = wbSheet(ExcelJS, "الإحصائيات");
...
export async function exportStudentsExcel(opts: {
  ...
  await downloadWorkbook(`طالبات-${opts.madrasa}.xlsx`, (ExcelJS) => {
    const ws = wbSheet(ExcelJS, "الطالبات");
...
export async function exportVolunteersExcel(opts: {
  ...
  await downloadWorkbook(`طاقم-${opts.madrasa}.xlsx`, (ExcelJS) => {
    const ws = wbSheet(ExcelJS, "الطاقم");
...
function wbSheet(ExcelJS: typeof import("exceljs").default, name: string) {
  const ws = ExcelJS.Workbook.prototype.addWorksheet.call(new ExcelJS.Workbook(), name);
  return ws;
}

function styleHeader(ws: import("exceljs").Worksheet, row: number) {
...
// إبقاء الفئات مستخدمة لتصدير التقارير حسب الفئة
export const REPORT_CATEGORY_KEYS = TRACK_CATEGORY_KEYS;
